...
const Registerpage = () => {
  ...
  const onSubmit = (data) => {
    ...
    formData.append("files", data.picture[0]); // Append image file to FormData
    formData.append("username", data.username); // Append other form fields to FormData
    formData.append("email", data.email);
    formData.append("password", data.password);

    // Replace the 'picture' field with the image file name
    data = { ...data, picture: data.picture[0].name };

    // Convert the 'data' object to JSON and append it to FormData with key "user"
    ...
    axiosClient
      .post("/utilisateur/register", formData)
      .then(({ data }) => {
        setUser(data.user);
